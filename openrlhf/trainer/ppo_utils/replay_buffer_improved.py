...
def split_experience_batch(experience: Experience_improved, data_processor: Optional[BaseDataProcessor]) -> List[BufferItem_improved]:
    ...
    items = [BufferItem_improved(**kwargs) for kwargs in batch_kwargs]
    return items
...
def make_experience_batch(items: List[BufferItem_improved], data_processor: Optional[BaseDataProcessor], packing_samples=False) -> Experience_improved:
    kwargs = {}
    keys = (
        "sequences",
        "action_log_probs",
    ...
    return Experience_improved(**kwargs)
...
class NaiveReplayBuffer_improved(ABC):
    ...
    def __init__(
        ...
        self.items: List[BufferItem_improved] = []
        self.maxlen = maxlen
    ...
    @torch.no_grad()
    def append(self, experience: Experience_improved) -> None:
        if self.cpu_offload:
    ...
    @torch.no_grad()
    def sample(self) -> Experience_improved:
    ...
    def __getitem__(self, idx: int) -> BufferItem_improved:
        return self.items[idx]

    def collate_fn(self, batch) -> Experience_improved:
