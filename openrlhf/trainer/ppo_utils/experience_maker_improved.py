...
class NaiveExperienceMaker_improved(ABC):
    """
    ...
    def make_experience_list(
    ...
    ) -> Tuple[Experience_improved, float]:
        """
        ...
        #print('samples_list',len(samples_list))
        for samples in tqdm(
            samples_list,
        ...
        # added
        #print('experiences[0]',experiences[0])
        accuracy_rewards_total = sum(e.info["acc_rewards"].sum() for e in experiences)
        accuracy_rewards_count = sum(e.info["acc_rewards"].numel() for e in experiences)
        ...
        if args.enable_accuracy_filter and global_step > args.freezing_filter_steps:
            print('(Filtering out experiences based on Gaussian Reward during training PRIME) begin filter')
        ...
        return experiences, accuracy_rewards_original.item() #added
    ...
    @torch.no_grad()
    def make_experience(self, samples: Samples) -> Experience_improved:
        ...
        if self.remote_rm_url is not None:
            ...
            print('samples.prompts',samples.prompts)
    ...
    @torch.no_grad()
    def filter(self, experiences: List[Experience_improved]) -> List[Experience_improved]:
        """
        Filter experiences based on gaussian reward variance with multiple strategies.
        """
        
        args = self.strategy.args
        gaussian_rewards = torch.cat([experience.info["acc_rewards"] for experience in experiences])
        gaussian_rewards = gaussian_rewards.reshape(-1, args.n_samples_per_prompt).to(device="cuda")# 16,8
        assert len(experiences) % gaussian_rewards.shape[0] == 0 # 128 / 16
        
        group_len = len(experiences) // gaussian_rewards.shape[0] #group_len 4
        grouped_experience = [experiences[i : i + group_len] for i in range(0, len(experiences), group_len)] #16个组
        
        # Calculate statistics for each group
        gaussian_stds = torch.std(gaussian_rewards, dim=-1)
        gaussian_means = torch.mean(gaussian_rewards, dim=-1)
        
        # Method 1: Adaptive threshold based on data distribution
        overall_std = torch.std(gaussian_means)  # std across different prompts
        adaptive_threshold = args.min_gaussian_std#max(0.001, overall_std.item() * 0.1)  # 10% of overall variation
        
        min_std_threshold = adaptive_threshold
        
        filtered_experiences = []
        valid_groups = 0
        
        print(f"Using adaptive_threshold: {min_std_threshold:.4f}")
        print(f"Std distribution: min={gaussian_stds.min():.4f}, max={gaussian_stds.max():.4f}, median={torch.median(gaussian_stds):.4f}")
        
        for i, (group, std_val, rewards) in enumerate(zip(grouped_experience, gaussian_stds, gaussian_rewards)):
            # Check std threshold
            if std_val.item() <= min_std_threshold:
                continue
        ...
        print(f"Kept {valid_groups}/{len(grouped_experience)} groups") # kept 4/16个组，所以是12个组倍丢弃，12*8=96个experience被丢弃
        if valid_groups > 0:
            kept_stds = gaussian_stds[gaussian_stds > min_std_threshold]
            print(f"Mean std of kept groups: {kept_stds.mean().item():.4f}")
    ...
    '''def filter(self, experiences: List[Experience_improved]) -> List[Experience_improved]:
        ...
        args = self.strategy.args
        accuracy_rewards = torch.cat([experience.info["acc_rewards"] for experience in experiences])
    ...
    @torch.no_grad()
    def process_experiences(self, experiences: List[Experience_improved]) -> Tuple[List[Experience_improved], List[torch.Tensor]]:
        """
        ...
        if args.use_adora:
            print('ADORA: Enable Adaptive Online Rollout Adjustment:')
            ...
            weights = weight_func_new(rewards, response_lengths, args.adora_lamda) #weighting different samples
            print('ADPRA Weight:',weights)
        else:
...
class RemoteExperienceMaker_improved(NaiveExperienceMaker_improved):
    ...
    def make_experience_list( #add
        ...
        experiences, accuracy_rewards_original  = super().make_experience_list(all_prompts, all_labels, global_step,**generate_kwargs)
        if self.critic is not None:
    ...
    def make_experience(self, samples: Samples) -> Experience_improved:
        ...
        experience = Experience_improved(
            sequences,
            action_log_probs,
            base_action_log_probs,
            value,
...
def weight_func(rewards, response_length, lamda=0.1):
    ...
    for i in range(rewards.shape[0]):
        ...
        response_reward_not_1 = response_row[(reward_row <= 0.5) & (response_row < 250)]
...
def weight_func_new(rewards, response_length, lamda=0.1, max_length=450):
    """
    Encourage short and correct responses.
    
    - rewards: Tensor [batch_size, seq_len], values in [0, 1]
    - response_length: Tensor [batch_size, seq_len], token lengths
    - lamda: penalty weight
    - max_length: the maximum length considered "short"
    """
    weights = torch.ones_like(rewards, device=rewards.device)

    for i in range(rewards.shape[0]):
        reward_row = rewards[i]
        length_row = response_length[i]

        # 找出 reward > 0.5 且长度小于 max_length 的响应
        good_and_short = (reward_row > 1.3) & (length_row < max_length)

        if good_and_short.any():
            # 有至少一个短且好的响应，保留权重 1
            continue
        else:
            # 否则降低整行权重
