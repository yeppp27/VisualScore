...
from .ppo_utils import Experience_improved, NaiveExperienceMaker_improved, NaiveReplayBuffer_improved


class PPOTrainer_improved(ABC):
    """
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        #self.actor_loss_fn = PolicyLoss(eps_clip, self.args.use_dapo, self.args.use_clip_higher,self.args.clip_eps_low,self.args.clip_eps_high)
        ...
        self.ptx_loss_fn = GPTLMLoss() #freezing_entropy_steps

        self.freezing_actor_steps = getattr(self.args, "freezing_actor_steps", -1)
        self.freezing_entropy_steps = getattr(self.args, "freezing_entropy_steps", -1)
        # Mixtral 8x7b
        ...
        self.experience_maker = NaiveExperienceMaker_improved(
            actor,
            critic,
            reward_model,
            initial_model,
        ...
        self.replay_buffer = NaiveReplayBuffer_improved(
            micro_train_batch_size,
            strategy,
            self.data_processor, buffer_limit, buffer_cpu_offload, packing_samples,
    ...
    def fit(
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, labels in self.prompts_dataloader:
                ...
                #    self.experience_maker.make_experience_list(rand_prompts, labels, steps, **self.generate_kwargs) #add steps for 
                #):
                #    if i == 0:
                ...
                if self.replay_buffer.is_full():
                    ...
                    status = {}
                status["accuracy_rewards_original"] = accuracy_rewards_original #added
    ...
    def training_step(self, experience: Experience_improved, global_steps) -> Dict[str, float]:
        status = {}
        if global_steps > self.freezing_actor_steps:
            if global_steps > self.freezing_entropy_steps:
                status = self.training_step_actor(experience,use_entropy=True)
            else:
                status = self.training_step_actor(experience,use_entropy=False)
        if self.critic is not None:
            status.update(self.training_step_critic(experience))
        return status

    def training_step_actor(self, experience: Experience_improved,use_entropy) -> Dict[str, float]:
        self.actor.train()
        ...
        response_token_ids = sequences[:, -response_len:]
    ...
    def training_step_critic(self, experience: Experience_improved) -> Dict[str, float]:
        self.critic.train()
