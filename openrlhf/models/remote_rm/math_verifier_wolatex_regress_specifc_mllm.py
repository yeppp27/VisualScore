...
def get_gpt_reward(response):
    ...
    prompt = f"""
You are a strict judge with one task: Determine if the Model's Response logically supports the Model's Answer.

# Decision Rules:
1. Output **1** when the reasoning in the response and the numerical answer clearly match in meaning and strength of evaluation.
2. Output **0** when there is a mismatch, such as:
   - The response gives an ambiguous or uncertain conclusion.
   - The conclusion contradicts the numerical score.
   - The response supports multiple possible scores equally.
   - The tone or evaluation in the response is significantly stronger or weaker than the final score.

# Notes:
- You are judging the consistency between reasoning and conclusion.
- Do NOT penalize for slight variation in language, as long as the meaning aligns.
- You may refer to general quality levels (bad, poor, fair, good, excellent).
...
def verify_conflict(prompt, image_paths=[], max_cycle=3):
    prompt = get_gpt_reward(prompt)
    url = "http://10.140.0.155:8001/v1/chat/completions"
    ...
    while max_cycle:
        try:
            ...
            answer = answer.replace("output:", "").strip()
            if len(answer) == 1 and answer in ['1', '0']:
...
def get_reward():
    ...
    g.args = app.config["ARGS"]  # 每次请求把 args 放到 g，线程安全
    # 检查是否有 'query' 字段
    ...
    for q, problem in zip(data["query"], data["prompts"]):
        ...
        acc_reward_future = math_verify_executor.submit(
            ...
            reward_type,
            dataset_name,
            problem  # 传递问题字符串用于数据集检测
        )
       
        do_print = random.randint(1, 20) == 1
        if do_print:
            info = f"Query: {q}\n\nProblem: {problem}\n\n Answer: {answer}\n\n Response: {response}\n\n Format Reward: {format_reward}\n\n Acc Reward: {acc_reward_future.result()}\n\n Conflict Reward: {conflict_reward}" 
            info = re.sub(r"<\|.*?\|>", "", info)
            print(info)
            
        format_rewards.append(format_reward)
        acc_rewards_futures.append(acc_reward_future )
        conflict_rewards.append(conflict_reward)
    
    acc_rewards = [f.result() for f in acc_rewards_futures]
    rewards = [f + a * (1 + 0.5 * c) for f, a , c in zip(format_rewards, acc_rewards, conflict_rewards)]
...
if __name__ == "__main__":
    ...
    app.config["ARGS"] = args  # ✅ 把 args 存到 app.config，线程安全
